...
Component({
  ...
  observers: {
      'show': function(val) {
          ...
          if (!width) return this.triggerEvent('error', {
              message: 'width不能为空'
          });
          if (!height) return this.triggerEvent('error', {
              message: 'height不能为空'
          });
          if (!bgImage) return this.triggerEvent('error', {
              message: 'bgImage不能为空'
          });
          if (!qrCode) return this.triggerEvent('error', {
              message: 'qrCode不能为空'
          });
  ...
  methods: {
      _initTimeline() {
          ...
          // 获取用户信息
          const getSystemInfo =  () => api('getSystemInfo')
              .then((res) => {
              // 设备像素比例(以iphone6s屏幕尺寸为基准)
                  let { pixelRatio } = res;
                  scale = pixelRatio;
